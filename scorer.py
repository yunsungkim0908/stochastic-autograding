...
class SkeletonScorer(Scorer):
    ...
    def rejection_threshold(self, frr, num_stud_samples, num_soln_samples, num_monte_carlo=1000):
        return self.rejection_threshold_fn(frr, num_stud_samples, num_soln_samples, num_monte_carlo=1000)
...
class AndersonDarlingScorer(Scorer):
    ...
    def compute_score(self, stud_samples, soln_samples):
        score = stats.anderson_ksamp([stud_samples, soln_samples]).statistic
        return score
...
class MSDScorer(Scorer):
    ...
    def compute_score(self, stud_samples, soln_samples):
        anchor = self.soln_samples[:self.num_soln_samples]
        true_E = np.mean((anchor - np.mean(anchor)) ** 2)
        true_std = np.std((anchor - np.mean(anchor)) ** 2) / np.sqrt(len(samples))
        sample_E = np.mean((samples - np.mean(anchor)) ** 2)
        score = abs((sample_E - true_E) / true_std)
        return score
